本次课程设计使用Openvino进行，安装的教程可以按照官方的教程来安装，安装的系统为Ubuntu系统，Openvino的版本是2019.1.1如下是安装的过程,本次实验通过胡厚淮同学的帮助下完成实验

本人的实验内容是基于Openvino的图像处理、检测与识别。识别内容是车辆目标，利用的是Openvino中已经提前训练好的小车识别
...
安装完成之后进入Openvino的sample文件夹进行测试指令如下
\# cd /opt/intel/openvino/..../samples
\# sudo mkdir build
\# cd build
\# cmake ..
\# make
其中过程截图如下
![](/image/Cmake_process.png)
![](/image/Cmake_Last.png)
![](/image/Make_process.png)
![](/image/Make_LAST.png)

然后在其中进去intel64，然后是Release文件夹，进行一个代码的测试。指令如下，我的指令是
\# ./security_barrier_demo -i /opt/intel/openvino_2018.1.094/deployment_tools/intel_models/vehition/vehicle-detection-adas-0002.png -m /opt /home/bosshu/Downloads/behicle-license-plate-detection-barrier-0106-fp16.xml
训练及结果图如下
![](/image/Test.png)
![](/image/Make_LAST.png)
![](/image/Vehicle_Detection.png)

小车识别代码如下，借用了Openvino中原来自带的main.cpp文件，文件中代码如下
```C++
...
bool ParseAndCheckCmmandLine(int argc, char *argv[]) {
...
再放一次结果图
![](/image/Test.png)
分析：Openvino中含有丰富的代码程序，和提前已经训练好模型供大家使用，而且其本身也是一个可以利用好深度训练的一款软件，非常适合用于无人驾驶方面的实习与应用

总结：第一次安装Openvino的时候挺顺利的，安装也成功了，后来因为教程的一些原因将，2020版本的Openvino替换成了2019版本的Openvino，之后再进行cmake的安装和Python的安装，不断的发生报错，不断的发生报错，后来只能借助于胡总的电脑进行实验，最后成功了，大部分是自己的做的，少部分靠别人吧。


心得体会：主要还是因为环境的配置问题。不管怎么说最后还是把答案做出来了还是可喜可贺的。Openvino是一款非常好的模型，内含有丰富的示例，而且代码是开源的，你可以从官网上下载很多。最后也挺心酸的但是，付出终归是有汇报，我觉得还是努力努力再努力，做一个好的打工人！！！