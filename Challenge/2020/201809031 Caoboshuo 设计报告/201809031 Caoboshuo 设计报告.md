...
![markdown](https://i0.hdslb.com/bfs/album/92207ae8f999752ab408aa9515ffa559d42ddc39.png@518w.png)

选择对应版本下载并安装

## 2 设置环境变量

通过在搜索栏输入cmd,打开管理员命令提示符，运行下列代码，临时设置环境变量。

C:\Program Files(x86)\IntelSWTools\openvino\bin\setupvars.bat

## 3 配置模型优化器 
   进入如下目录C:\Program Files (x86)\IntelSWTools\openvino\deployment_tools\model_optimizer\install_prerequisites中，运行install_prerequisites.bat
...
如图所示，汽车显示出其方框区域，颜色及其归属地等属性信息，完成了对汽车属性信息的识别。

![markdown](https://i0.hdslb.com/bfs/album/6ca8f0844cf38f96934530bbc047bab5566e3179.png)


![markdown](https://i0.hdslb.com/bfs/album/7c8c17cfdbb56ad3f5d2ddc55a1030974c20b1d3.png@518w.png)


分析：
OpenVINO是英特尔基于自身现有的硬件平台开发的一种可以加快高性能计算机视觉和深度学习视觉应用开发速度工具套件，支持各种英特尔平台的硬件加速器上进行深度学习，并且允许直接异构执行。 支持在Windows与Linux系统，Python/C++语言。在实验中通过使用OpenVINO的视觉识别能力来分析并得出车辆的属性信息。


总结：
本实验基于OpenVINO的图像处理检测和识别对行驶车辆进行识别，识别出车的自身属性。借助了OpenVINO的计算机视觉和深度学习应用开发的功能， OpenVINO提供的模型功能强大，通过模型训练可以解决许多的问题。

心得：
通过这次实验掌握了Windows安装使用OpenVINO的基本流程，在配置好环境文件和所检测的图片后，通过使用OpenVINO进行识别车辆图片，得出图片中车辆的属性信息。通过实验进一步的了解了OpenVINO的相关功能特性，在以后的学习和工作中，也会进一步的学习OpenVINO，并不断地应用实践。
